...
final class TeamController extends Controller
{
  ...
  /**
   * Display a listing of team members for the current company.
   */
  /*public function index(Request $request): Response
  {
    $company = $request->user()->currentCompany;
    ...
    Gate::authorize('viewTeamMembers', $company);

    // Get team members with their roles and pivot data
    $teamMembers = $company->users()
    ...
    $pendingInvitations = $company->invitations()
      ...
      ->get()
      ->map(function ($invitation) use ($request) {
        return [
          'id' => $invitation->id,
          'name' => $invitation->name,
          'email' => $invitation->email,
          ...
          'created_at' => $invitation->created_at->format('M d, Y'),
          'can' => [
            'cancel' => $request->user()->can('manageInvitations', [CompanyTeamPolicy::class, $company]),
          ],
        ];
      });

    // Get available roles (excluding super_admin and company_owner for UI)
    $roles = Role::whereNotIn('name', ['super_admin', 'company_owner'])
      ->get()
      ->map(function ($role) {
        return [
    ...
    return Inertia::render('team/Index', [
      ...
      'roles' => $roles,
      // Remove userPermissions - these now come from the centralized AuthorizationService via middleware
      // The frontend will use $page.props.auth.user.abilities instead
    ]);
  }*/
  ...
  public function index(Request $request): Response
  {
    ...
    // Get team members with proper relationships
    $teamMembersCollection = $company->users()
      ->with(['roles.permissions'])
      ->withPivot(['role', 'is_owner', 'joined_at'])
      ->orderBy('name')
      ->get()
      ->map(function ($user) use ($company, $request) {
        return [
          ...
          'name' => $user->name,
          'email' => $user->email,
          'role' => $user->pivot->role ?? $user->roles->first()?->name ?? 'viewer',
          'is_owner' => (bool)($user->pivot->is_owner ?? false),
          'avatar' => $user->profile_photo_url ?? $user->avatar ?? null,
          'joined_at' => $user->pivot->created_at?->format('M d, Y') ?? $user->created_at?->format('M d, Y'),
          'can' => [
            'edit' => $this->canUpdateTeamMember($request->user(), $company, $user),
            'delete' => $this->canRemoveTeamMember($request->user(), $company, $user),
          ],
        ];
      });
    ...
    $pendingInvitations = $company->invitations()
      ...
      ->map(function ($invitation) use ($request, $company) {
        return [
          ...
          'created_at' => $invitation->created_at->format('M d, Y'),
          'can' => [
            'cancel' => $this->canManageInvitations($request->user(), $company),
          ],
        ];
      });
    ...
    return Inertia::render('team/Index', [
      'team' => new TeamResource($teamMembersCollection), // This will return { data: [...] }
      'invitations' => $pendingInvitations->all(),
      'company' => [
        'id' => $company->id,
        'name' => $company->name,
      ],
...
// Authorization helper methods (same as before)
  private function canUpdateTeamMember(User $user, Company $company, User $member): bool
  {
    ...
    if (!($member->pivot->is_owner ?? false)) {
  ...
  private function canRemoveTeamMember(User $user, Company $company, User $member): bool
  {
    if (!$user->canAccessCompany($company)) {
      return false;
    }
  ...
  private function canManageInvitations(User $user, Company $company): bool
  {
